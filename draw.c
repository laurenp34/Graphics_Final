...
//adds a cylinder pointing out in z axis to points matrix. height represents change in z coordinate.
void add_cylinder(struct matrix * edges, double cx, double cy, double cz, double radius, double height, int step) {
  struct matrix *points = generate_cylinder(cx,cy,cz, radius, height, step);
  int p0, p1, p2, p3, lat, longt;
  int latStop, longStop, latStart, longStart;
  latStart = 0;
  latStop = step;
...
//adds points (edges) for vertical cylinder to struct matrix * points, returns points
struct matrix * generate_cylinder(double cx, double cy, double cz, double radius, double height, int step) {
  struct matrix * points = new_matrix(4, step*step);
  int circle, up, circ_start,circ_end, up_start,up_end;
  double z = cz;
  ...
  for (up = up_start; up<up_end; up++) {
    z += (double) height/step;
    printf("new z: %f (+ %f)\n", z, (double)height/step);
    add_circle(points, cx,cy,z, radius, step);
  }
  printf("final z: %f\t supposed to be: %f\n",z, (cz+height));
  return points;
}
